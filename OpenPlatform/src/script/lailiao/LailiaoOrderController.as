package script.lailiao
{
	...
	public class LailiaoOrderController extends Script
	{
		private var uiSkin:LailiaoOrderPanelUI;
		public var param:Object;
		
		private var orderlist:Vector.<LailiaoOrderItemCell>;
		private var mianvbox:Number = 0;
		
		private var requestnum:int = 0;
		
		public function LailiaoOrderController()
		{
			super();
		}
		...
		override public function onStart():void
		{
			...
			orderlist = new Vector.<LailiaoOrderItemCell>();
			...
			if(PaintOrderModel.instance.tempPicOrderItemVoList != null)
			{
				for(var j:int=0;j < PaintOrderModel.instance.tempPicOrderItemVoList.length;j++)
				{
					...
					var item:LailiaoOrderItemCell = new LailiaoOrderItemCell(PaintOrderModel.instance.tempPicOrderItemVoList[j]);
			...
			for each(var fvo in DirectoryFileModel.instance.haselectPic)
			{
				if(fvo.colorspace.toUpperCase() == "GRAY")
				{
					...
					ovo.indexNum = i++;
					var item:LailiaoOrderItemCell = new LailiaoOrderItemCell(ovo);
					uiSkin.ordervbox.addChild(item);
					orderlist.push(item);
					if(num > 0)
						uiSkin.ordervbox.height += item.height + uiSkin.ordervbox.space;
			...
			uiSkin.addpic.on(Event.CLICK,this,addLailiaoItem);
		...
		private function addLailiaoItem():void
		{
			var fvo:LailiaoPicInfo = new LailiaoPicInfo({},1,OrderConstant.orderName[PaintOrderModel.instance.orderType] + "产品");
			var ovo:PicOrderItemVo = new PicOrderItemVo(fvo,PicOrderItemVo.LAILIAO_ITEM);
			...
			{
				if(i == 0)
					(uiSkin.ordervbox.getChildAt(0) as LailiaoOrderItemCell).y = 0;
				else
					(uiSkin.ordervbox.getChildAt(i) as LailiaoOrderItemCell).y = (uiSkin.ordervbox.getChildAt(i-1) as LailiaoOrderItemCell).y + (uiSkin.ordervbox.getChildAt(i-1) as LailiaoOrderItemCell).height + uiSkin.ordervbox.space;
			}
		...
		private function resetOrderInfo():void
		{
			...
			if(orderlist.length > 0)
			{
				...
				for(var i:int=0; i < orderlist.length;i++)
				{
					var orderitem:LailiaoOrderItemCell = orderlist[i];
		...
		private function onDeletePicOrder(orderitem:LailiaoOrderItemCell):void
		{
			...
			for(var i:int=0;i < uiSkin.ordervbox.numChildren;i++)
			{
				var pvo:PicOrderItemVo = (uiSkin.ordervbox.getChildAt(i) as LailiaoOrderItemCell).ordervo;
				if(pvo.indexNum > ordervo.indexNum)
					pvo.indexNum--;
				(uiSkin.ordervbox.getChildAt(i) as LailiaoOrderItemCell).updateIndex();
			}
		...
		{			
			...
			PaintOrderModel.instance.batchChangeMatItems = new Vector.<LailiaoOrderItemCell>();
			for(var i:int=0;i < orderlist.length;i++)
			{
				if(orderlist[i].checkSel.selected)
				{
