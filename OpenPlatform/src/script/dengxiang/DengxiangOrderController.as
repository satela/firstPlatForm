package script.dengxiang
{
	...
	import model.dengxiangModel.DengxiangPicInfo;
	import model.lailiaoModel.LailiaoPicInfo;
	...
	public class DengxiangOrderController extends Script
	{
		private var uiSkin:LailiaoOrderPanelUI;
		public var param:Object;
		
		private var orderlist:Vector.<DengxiangOrderItemCell>;
		private var mianvbox:Number = 0;
		
		private var requestnum:int = 0;
		
		public function DengxiangOrderController()
		{
			super();
		}
		...
		override public function onStart():void
		{
			...
			orderlist = new Vector.<DengxiangOrderItemCell>();
			...
			if(PaintOrderModel.instance.tempPicOrderItemVoList != null)
			{
				for(var j:int=0;j < PaintOrderModel.instance.tempPicOrderItemVoList.length;j++)
				{
					...
					var item:DengxiangOrderItemCell = new DengxiangOrderItemCell(PaintOrderModel.instance.tempPicOrderItemVoList[j]);
...
//			{
//				if(fvo.colorspace.toUpperCase() == "GRAY")
//				{
//					var ovo:PicOrderItemVo = new PicOrderItemVo(fvo);
//					ovo.indexNum = i++;
//					var item:DengxiangOrderItemCell = new DengxiangOrderItemCell(ovo);
//					uiSkin.ordervbox.addChild(item);
//					orderlist.push(item);
//					if(num > 0)
//						uiSkin.ordervbox.height += item.height + uiSkin.ordervbox.space;
...
//			}
			...
			uiSkin.addpic.on(Event.CLICK,this,addDengxiangItem,[0]);
			uiSkin.btnaddpic.on(Event.CLICK,this,addDengxiangItem,[1]);
		...
		private function addDengxiangItem(type:int):void
		{
			var fvo:DengxiangPicInfo = new DengxiangPicInfo({},1,["矩形","圆形"][type] + OrderConstant.orderName[PaintOrderModel.instance.orderType] + "产品",type);
			var ovo:PicOrderItemVo = new PicOrderItemVo(fvo,PicOrderItemVo.DENGXIANG_ITEM);
			...
			if(orderlist.length +  1 > OrderConstant.MAX_ORDER_NUMER)
			{
			...
			{
				if(i == 0)
					(uiSkin.ordervbox.getChildAt(0) as DengxiangOrderItemCell).y = 0;
				else
					(uiSkin.ordervbox.getChildAt(i) as DengxiangOrderItemCell).y = (uiSkin.ordervbox.getChildAt(i-1) as DengxiangOrderItemCell).y + (uiSkin.ordervbox.getChildAt(i-1) as DengxiangOrderItemCell).height + uiSkin.ordervbox.space;
			}
		...
		private function resetOrderInfo():void
		{
			...
			if(orderlist.length > 0)
			{
				...
				for(var i:int=0; i < orderlist.length;i++)
				{
					var orderitem:DengxiangOrderItemCell = orderlist[i];
		...
		private function onDeletePicOrder(orderitem:DengxiangOrderItemCell):void
		{
			...
			for(var i:int=0;i < uiSkin.ordervbox.numChildren;i++)
			{
				var pvo:PicOrderItemVo = (uiSkin.ordervbox.getChildAt(i) as DengxiangOrderItemCell).ordervo;
				if(pvo.indexNum > ordervo.indexNum)
					pvo.indexNum--;
				(uiSkin.ordervbox.getChildAt(i) as DengxiangOrderItemCell).updateIndex();
			}
		...
		{			
			...
			PaintOrderModel.instance.batchChangeMatItems = new Vector.<DengxiangOrderItemCell>();
			for(var i:int=0;i < orderlist.length;i++)
			{
				if(orderlist[i].checkSel.selected)
				{
