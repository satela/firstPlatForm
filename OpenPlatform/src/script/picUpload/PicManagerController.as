package script.picUpload
{
	...
	public class PicManagerController extends Script
	{
		private var uiSkin:PicManagerPanelUI;
		...
		override public function onStart():void
		{
			...
			directTree = [];
			uiSkin = this.owner as PicManagerPanelUI; 
...
//			uiSkin.btnCloseInput.on(Event.CLICK,this,onCloseCreateFolder);
			...
			uiSkin.btnorder.on(Event.CLICK,this,function(){
				...
				EventCenter.instance.event(EventCenter.SHOW_CONTENT_PANEL,[OrderTypePanelUI,0,curCustomer]);
			...
			EventCenter.instance.on(EventCenter.UPLOAD_FILE_SUCESS,this,uploadFileSucess);
...
//				}
		...
		private function onGetDirFileListBack(data:Object):void
		{
			...
			if(DirectoryFileModel.instance.hasUnDealPic())
			{
				//Laya.timer.loop(2000,this,refreshFileList);
			}
			else
			{
				Laya.timer.clear(this,refreshFileList);
		...
		private function uploadFileSucess():void
		{
			autoFreshTime = 0;
			getFileList();
			Laya.timer.loop(2000,this,refreshFileList);
